...
void blendColors(led_t* color1, led_t* color2, led_t* destColor) {
    destColor->red   = (((int)color1->red   + color2->red) / 2);
    destColor->green = (((int)color1->green + color2->green) / 2);
    destColor->blue  = (((int)color1->blue  + color2->blue) / 2);
}
...
static void effect_weave_init(void) {
...
////// Blinking Effect //////
/*
static led_t blinkColor = yellow;
// 0 means that it is not connecting
static uint8_t bltState = 0;
static pos_i blinkPos;
static bool blinkingEnabled = false;
static bool blinkingState = false;
static systime_t blinkingSpeed = 300;
static systime_t blinkingTime;

void effect_blt_init(uint8_t bltState) {
    effect_blinking_init(NULL);
    const led_t bltColor = {0, 255, 30};
    blinkColor = bltColor;
    blinkPos.x = (bltState - 1) % 4 + 1;
    blinkPos.y = 0;
...
bool effect_blinking_tick(led_t* ledColors) {
    ...
    if (!blinkingEnabled) return false;

    const systime_t currTime = sysTimeMs();
    if (currTime - blinkingTime > blinkingSpeed) {
        blinkingTime = currTime;
        blinkingState;
    }

    if (blinkingState) {
        setColor(ledColors, &blinkPos, &blinkColor);
    }
