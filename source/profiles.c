...
void redProfile(led_t* ledColors){
  setAllKeysColor(ledColors, 0xFF0000);
}

void greenProfile(led_t* ledColors){
  setAllKeysColor(ledColors, 0x00FF00);
}

void blueProfile(led_t* ledColors){
  setAllKeysColor(ledColors, 0x0000FF);
}

void miamiNights(led_t* ledColors){
  setAllKeysColor(ledColors, 0x00979c);
  setModKeysColor(ledColors, 0x9c008f);
}

void rainbowHorizontal(led_t* ledColors){
...
void rainbowVertical(led_t* ledColors){
...
void animatedRainbowVertical(led_t* ledColors){
  for (uint16_t i=0; i<NUM_COLUMN; ++i){
    for (uint16_t j=0; j<NUM_ROW; ++j){
...
void animatedRainbowWaterfall(led_t* ledColors){
  for(int i = 0; i < NUM_ROW; i++){
    setRowColorHSV(ledColors, i, waterfallValue[i], 255, 125);
...
void animatedBreathing(led_t* ledColors){
  setAllKeysColorHSV(ledColors, 85, 255, breathingValue);
...
void animatedSpectrum(led_t* ledColors){
  setAllKeysColorHSV(ledColors, spectrumValue, 255, 125);
